...
class FutureGenerator(nn.Module):
    ...
    def to_rgb_block(self, ndim):
        ...
        layers = []
        layers = deconv(layers, ndim, self.nc_out, 1, 1, 0, self.padding, self.lrelu, self.batch_norm, self.w_norm, self.g_pixelwise_norm, only=True)
    ...
    def from_rgb_block(self, ndim):
        ...
        layers = []
        layers = conv(layers, self.nc_in, ndim, 1, 1, 0, self.padding, self.lrelu, self.batch_norm, self.w_norm, self.d_gdrop, self.g_pixelwise_norm)
        return  nn.Sequential(*layers)
    ...
    def forward(self, x):
        ...
        if self.config.d_cond==True:
            y = self.model(x[:,:self.nc_in,:,:,:]) # let the model only see the specified number of channels from the input data
            if self.nc_in == self.nc_out:
                x = torch.cat([x, y],2)
            elif self.nc_in < self.nc_out:
                x = torch.cat([x, y], 2)
            elif self.nc_in > self.nc_out:
                x = torch.cat([x[:,:self.nc_out,:,:,:], y],2) #[:,:self.nc_in,:self.nframes_in,:,:] # Seq, Channels, Frames, H, W
        else:
            x = self.model(x)
...
class Discriminator(nn.Module):
    ...
    def from_rgb_block(self, ndim):
        ...
        layers = []
        layers = conv(layers, self.nc_out, ndim, 1, 1, 0, self.padding, self.lrelu, self.batch_norm, self.w_norm, self.d_gdrop, pixel_norm=False)
        return  nn.Sequential(*layers)
