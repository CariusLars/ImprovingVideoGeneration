...
from utils import get_image_grid, save_image_grid, get_image_grid_depth, save_image_grid_depth, count_model_params
...
parser.add_argument('--nc_in', type=int, default=3, help='number of input image channels, default=3')
parser.add_argument('--nc_out', type=int, default=3, help='number of input image channels, default=3')
...
parser.add_argument('--metrics_depth', action='append', help='list of evaluation metrics to calculate for depth maps(choices: `mse`, `psnr`), default=``')
...
def evaluate_pred(config):
    ...
    if config.model=='FutureGAN' or config.model=='CopyLast':
        ...
        dataset_gt = DeformDataset(data_root_path=test_data_root, snippet_length=nframes,transform=transform, channels=4)
        sampler = DeformDatasetSampler(dataset_gt, None, nframes)
        dataloader_gt = DataLoader(dataset=dataset_gt, batch_size=config.batch_size, num_workers=nworkers, sampler=sampler, pin_memory=True)
        ...
        #dataloader_gt = DataLoader(dataset=dataset_gt, batch_size=config.batch_size, sampler=sampler.SequentialSampler(dataset_gt), num_workers=nworkers)
        dataset_gt = DeformDataset(data_root_path=test_data_root+'/in_gt', snippet_length=nframes, transform=transform, channels=4)
        sampler_gt = DeformDatasetSampler(dataset_gt, None, nframes)
        dataloader_gt = DataLoader(dataset=dataset_gt, batch_size=config.batch_size, num_workers=nworkers, sampler=sampler_gt, pin_memory=True)
        dataset_pred = DeformDataset(data_root_path=test_data_root + '/in_pred', snippet_length=nframes,transform=transform, channels=4)
        sampler_pred = DeformDatasetSampler(dataset_pred, None, nframes)
        dataloader_pred = DataLoader(dataset=dataset_pred, batch_size=config.batch_size, num_workers=nworkers,sampler=sampler_pred, pin_memory=True)
    ...
    z = Variable(torch.FloatTensor(config.batch_size, config.nc_in, nframes_in, img_size, img_size))
    z_in = Variable(torch.FloatTensor(config.batch_size, config.nc_in, nframes_in, img_size, img_size))
    x_pred = Variable(torch.FloatTensor(config.batch_size, config.nc_out, nframes_pred, img_size, img_size))
    x = Variable(torch.FloatTensor(config.batch_size, 4, nframes, img_size, img_size))
    x_eval = Variable(torch.FloatTensor(config.batch_size, config.nc_out, nframes_pred, img_size, img_size))
    ...
    if config.metrics_depth is not None:
        assert config.nc_out > 3, "Depth metrics cannot be evaluated if there is no depth channel in the prediction"
        metrics_values_depth = {}
        for metric_name in config.metrics_depth:
            metrics_values_depth['{}_frames'.format(metric_name)] = torch.zeros_like(torch.FloatTensor(test_len, nframes_pred))
            metrics_values_depth['{}_avg'.format(metric_name)] = torch.zeros_like(torch.FloatTensor(test_len, 1))
            print(' ... calculating {}_depth ...'.format(metric_name))
    ...
    if config.metrics_depth is not None:
        metrics_i_video_depth = {}
        for metric_name in config.metrics_depth:
            metrics_i_video_depth['{}_i_video'.format(metric_name)] = 0

    i_save_video = 1
    i_save_gif = 1
    i_save_video_depth = 1
    i_save_gif_depth = 1
    ...
    for step in tqdm(range(len(data_iter_gt))):
        ...
        x_eval.data = x.data[:,:config.nc_out,nframes_in:,:,:] # end part
        z.data = x.data[:,:config.nc_in,:nframes_in,:,:] # beginning part
        ...
        if config.model=='FutureGAN':
            ...
            if config.deep_pred > 1:
                for i_deep_pred in range(0,config.deep_pred):
                    ...
                    if config.nc_in <= config.nc_out:
                        z_in.data = x_pred.data[:,:config.nc_in,i_deep_pred*config.nframes_pred:(i_deep_pred*config.nframes_pred)+config.nframes_pred,:,:]
                    else:
                        print("Recursive prediction is only possible if input channels is smaller or equal to output channels")
                        raise NotImplementedError
            else:
                x_pred[:z_in.size(0), :,:, :, :] = G(z_in).detach()


        elif config.model=='CopyLast':
            for i_baseline_frame in range(x_pred.size(2)):
                x_pred.data[:x.size(0),:,i_baseline_frame,:,:] = x.data[:,:config.nc_out,nframes_in-1,:,:]

        else:
            x_pred.data = next(data_iter_pred)[:x.size(0),:config.nc_out,nframes_in:,:,:]
            ...
            for metric_name in config.metrics:
                ...
                for i_batch in range(x.size(0)):
                    for i_frame in range(nframes_pred):
                        metrics_values['{}_frames'.format(metric_name)][metrics_i_video['{}_i_video'.format(metric_name)], i_frame] = calculate_metric(x_pred[i_batch,:3,i_frame,:,:], x_eval[i_batch,:3,i_frame,:,:])
        ...
        # calculate depth eval statistics
        if config.metrics_depth is not None:
            for metric_name in config.metrics_depth:
                calculate_metric = getattr(eval_metrics, 'calculate_{}'.format(metric_name))
                ...
                for i_batch in range(x.size(0)):
                    for i_frame in range(nframes_pred):
                        metrics_values_depth['{}_frames'.format(metric_name)][metrics_i_video_depth['{}_i_video'.format(metric_name)], i_frame] = calculate_metric(x_pred[i_batch, np.newaxis, 3, i_frame, :, :], x_eval[i_batch, np.newaxis, 3, i_frame, :, :])
                        metrics_values_depth['{}_avg'.format(metric_name)][metrics_i_video_depth['{}_i_video'.format(metric_name)]] = torch.mean(metrics_values_depth['{}_frames'.format(metric_name)][metrics_i_video_depth['{}_i_video'.format(metric_name)]])
                    metrics_i_video_depth['{}_i_video'.format(metric_name)] = metrics_i_video_depth['{}_i_video'.format(metric_name)] + 1
            ...
            if step%config.save_frames_every==0 or step==0:
                for i_save_batch in range(x.size(0)):
                    ...
                    for i_save_z in range(z.size(2)):
                        save_image_grid(z.data[i_save_batch,:3,i_save_z,:,:].unsqueeze(0), sample_dir+'/in_gt/video{:04d}/video{:04d}_frame{:04d}_R{}x{}.png'.format(i_save_video, i_save_video, i_save_z+1, img_size, img_size), img_size, 1)
                        save_image_grid(z.data[i_save_batch,:3,i_save_z,:,:].unsqueeze(0), sample_dir+'/in_pred/video{:04d}/video{:04d}_frame{:04d}_R{}x{}.png'.format(i_save_video, i_save_video, i_save_z+1, img_size, img_size), img_size, 1)
                    for i_save_x_pred in range(x_pred.size(2)):
                        save_image_grid(x_eval.data[i_save_batch,:3,i_save_x_pred,:,:].unsqueeze(0), sample_dir+'/in_gt/video{:04d}/video{:04d}_frame{:04d}_R{}x{}.png'.format(i_save_video, i_save_video, i_save_x_pred+1+nframes_in, img_size, img_size), img_size, 1)
                        save_image_grid(x_pred.data[i_save_batch,:3,i_save_x_pred,:,:].unsqueeze(0), sample_dir+'/in_pred/video{:04d}/video{:04d}_frame{:04d}_R{}x{}.png'.format(i_save_video, i_save_video, i_save_x_pred+1+nframes_in, img_size, img_size), img_size, 1)
                    i_save_video = i_save_video+1

        # save depth maps
        if config.nc_out > 3 or config.nc_in > 3:
            if config.save_frames_every is not 0 and config.model == 'FutureGAN':
                if step % config.save_frames_every == 0 or step == 0:
                    for i_save_batch in range(x.size(0)):
                        if not os.path.exists(sample_dir + '/in_gt/video{:04d}'.format(i_save_video_depth)):
                            os.makedirs(sample_dir + '/in_gt/video{:04d}'.format(i_save_video_depth))
                        if not os.path.exists(sample_dir + '/in_pred/video{:04d}'.format(i_save_video_depth)):
                            os.makedirs(sample_dir + '/in_pred/video{:04d}'.format(i_save_video_depth))
                        for i_save_z in range(z.size(2)):
                            if config.nc_in > 3:
                                save_image_grid_depth(z.data[i_save_batch, 3, i_save_z, :, :],sample_dir + '/in_gt/video{:04d}/video{:04d}_frame{:04d}_R{}x{}_depth.pgm'.format(i_save_video_depth, i_save_video_depth, i_save_z + 1, img_size, img_size),img_size, 1)
                                save_image_grid_depth(z.data[i_save_batch, 3, i_save_z, :, :],sample_dir + '/in_pred/video{:04d}/video{:04d}_frame{:04d}_R{}x{}_depth.pgm'.format(i_save_video_depth, i_save_video_depth, i_save_z + 1, img_size, img_size),img_size, 1)
                        for i_save_x_pred in range(x_pred.size(2)):
                            if config.nc_out > 3:
                                save_image_grid_depth(x_eval.data[i_save_batch, 3, i_save_x_pred, :, :],sample_dir + '/in_gt/video{:04d}/video{:04d}_frame{:04d}_R{}x{}_depth.pgm'.format(i_save_video_depth, i_save_video_depth, i_save_x_pred + 1 + nframes_in,img_size, img_size), img_size, 1)
                                save_image_grid_depth(x_pred.data[i_save_batch, 3, i_save_x_pred, :, :],sample_dir + '/in_pred/video{:04d}/video{:04d}_frame{:04d}_R{}x{}_depth.pgm'.format(i_save_video_depth, i_save_video_depth, i_save_x_pred + 1 + nframes_in,img_size, img_size), img_size, 1)
                        i_save_video_depth = i_save_video_depth + 1
        ...
        if config.save_gif_every is not 0:
            if step%config.save_gif_every==0 or step==0:
                for i_save_batch in range(x.size(0)):
                    ...
                    frames = []
                    for i_save_z in range(z.size(2)):
                        frames.append(get_image_grid(z.data[i_save_batch,:3,i_save_z,:,:].unsqueeze(0), img_size, 1, config.in_border, config.npx_border))
                    for i_save_x_pred in range(x_pred.size(2)):
        ...
        if config.nc_out > 3 or config.nc_in > 3:
            if config.save_gif_every is not 0:
                if step%config.save_gif_every==0 or step==0:
                    for i_save_batch in range(x.size(0)):
                        if not os.path.exists(sample_dir+'/in_gt/video{:04d}'.format(i_save_gif_depth)):
                            os.makedirs(sample_dir+'/in_gt/video{:04d}'.format(i_save_gif_depth))
                        if not os.path.exists(sample_dir+'/in_pred/video{:04d}'.format(i_save_gif_depth)):
                            os.makedirs(sample_dir+'/in_pred/video{:04d}'.format(i_save_gif_depth))
                        if config.nc_in > 3:
                            frames = []
                            for i_save_z in range(z.size(2)):
                                frames.append(get_image_grid_depth(z.data[i_save_batch,3,i_save_z,:,:].unsqueeze(0), img_size, 1, config.in_border, config.npx_border))
                            for i_save_x_pred in range(x_pred.size(2)):
                                frames.append(get_image_grid_depth(x.data[i_save_batch,3,nframes_in+i_save_x_pred,:,:].unsqueeze(0), img_size, 1, config.out_border, config.npx_border))
                            imageio.mimsave(sample_dir+'/in_gt/video{:04d}/video{:04d}_R{}x{}_depth.gif'.format(i_save_gif_depth, i_save_gif_depth, img_size, img_size), frames)
                            #Image.fromarray(frames[0]*0.0001).save(sample_dir+'/in_gt/video{:04d}/video{:04d}_R{}x{}_depth.gif'.format(i_save_gif_depth, i_save_gif_depth, img_size, img_size), save_all=True, append_images=[Image.fromarray(next_frame*0.0001) for next_frame in frames])
                        if config.nc_out > 3:
                            frames = []
                            for i_save_z in range(z.size(2)):
                                frames.append(get_image_grid_depth(x.data[i_save_batch,3,nframes_in+i_save_z,:,:].unsqueeze(0), img_size, 1, config.in_border, config.npx_border))
                            for i_save_x_pred in range(x_pred.size(2)):
                                frames.append(get_image_grid_depth(x_pred.data[i_save_batch,3,i_save_x_pred,:,:].unsqueeze(0), img_size, 1, config.out_border, config.npx_border))
                            imageio.mimsave(sample_dir+'/in_pred/video{:04d}/video{:04d}_R{}x{}_depth.gif'.format(i_save_gif_depth, i_save_gif_depth, img_size, img_size), frames)
                            #Image.fromarray(frames[0]*0.0001).save(sample_dir + '/in_pred/video{:04d}/video{:04d}_R{}x{}_depth.gif'.format(i_save_gif_depth,i_save_gif_depth,img_size, img_size),save_all=True, append_images=[Image.fromarray(next_frame*0.0001) for next_frame in frames])
                        i_save_gif_depth = i_save_gif_depth+1
    ...
    # calculate and save depth mean eval statistics
    if config.metrics_depth is not None:
        metrics_mean_values_depth = {}
        for metric_name in config.metrics_depth:
            metrics_mean_values_depth['{}_frames'.format(metric_name)] = torch.mean(metrics_values_depth['{}_frames'.format(metric_name)],0)
            metrics_mean_values_depth['{}_avg'.format(metric_name)] = torch.mean(metrics_values_depth['{}_avg'.format(metric_name)],0)
            torch.save(metrics_mean_values_depth['{}_frames'.format(metric_name)], os.path.join(test_dir, '{}_frames_depth.pt'.format(metric_name)))
            torch.save(metrics_mean_values_depth['{}_avg'.format(metric_name)], os.path.join(test_dir, '{}_avg_depth.pt'.format(metric_name)))

        print(' ... saving depth evaluation statistics to dir: {}'.format(test_dir))
