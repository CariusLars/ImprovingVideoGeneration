...
def latentVectorInterpolate(input_vectors, output_count = 3, method = "linear", rank = None):
    ...
    if method == "linear" and len(input_vectors) == 2:
        ...
        t = np.arange(len(input_vectors))  # we assume the samples given are evenly distributed in time
        coefficients = np.zeros((input_vectors[0].shape[0], 2))
        for dimension in range(input_vectors[0].shape[0]):
            coefficients[dimension, :] = np.polyfit(t, [latent_vector[dimension] for latent_vector in input_vectors], 1)  # highest power coefficient first
        output_vectors = list()
        ...
        for dimension in range(input_vectors[0].shape[0]):
            coefficients[dimension, :] = np.polyfit(t, [latent_vector[dimension] for latent_vector in input_vectors], 2) # highest power coefficient first
        output_vectors = list()
        for step in np.linspace(0, len(input_vectors)-1, num=output_count):
        ...
        for dimension in range(input_vectors[0].shape[0]):
            coefficients[dimension, :] = np.polyfit(t, [latent_vector[dimension] for latent_vector in input_vectors],3)  # highest power coefficient first
        output_vectors = list()
        for step in np.linspace(0, len(input_vectors)-1, num=output_count):
            current_vector = coefficients[:, 0] * step ** 3 + coefficients[:, 1] * step ** 2 + coefficients[:, 2] * step + coefficients[:, 3]
            output_vectors.append(current_vector)
        assert len(output_vectors) == output_count
        return output_vectors
        ...
        for dimension in range(input_vectors[0].shape[0]):
            coefficients[dimension, :] = np.polyfit(t, [latent_vector[dimension] for latent_vector in input_vectors],rank)  # highest power coefficient first
        output_vectors = list()
        for step in np.linspace(0, len(input_vectors) - 1, num=output_count):
...
def latentVectorExtrapolateForward(input_vectors, output_count = 3, method = "linear", step_ratio = 1):
    """
    Function to fit a polynomial to given latent vectors and generate new latent vectors later in time than the given ones.
    :param input_vectors: list of 2 or more given latent vectors
    :param output_count: total number of desired output latent vectors (including the input vectors)
    ...
    if method == "linear" and len(input_vectors) == 2:
        ...
        coefficients = np.zeros((input_vectors[0].shape[0], 2))
        for dimension in range(input_vectors[0].shape[0]):
            coefficients[dimension, :] = np.polyfit(t, [latent_vector[dimension] for latent_vector in input_vectors],
                                                    1)  # highest power coefficient first
        output_vectors = list()
        for step in np.linspace(0, output_count-1, num=output_count):
            if step >= len(input_vectors):
                current_vector = coefficients[:, 0] * (len(input_vectors) - 1 + (step - len(input_vectors) + 1) * step_ratio) + coefficients[:, 1]
            else:
                current_vector = coefficients[:, 0] * step + coefficients[:, 1]
            output_vectors.append(current_vector)
        assert len(output_vectors) == output_count
        ...
        for dimension in range(input_vectors[0].shape[0]):
            coefficients[dimension, :] = np.polyfit(t, [latent_vector[dimension] for latent_vector in input_vectors],
                                                    2)  # highest power coefficient first
        output_vectors = list()
        for step in np.linspace(0, output_count - 1, num=output_count):
            if step >= len(input_vectors):
                current_vector = coefficients[:, 0] * (len(input_vectors) - 1 + (step - len(input_vectors) + 1) * step_ratio) ** 2 + coefficients[:, 1] * (len(input_vectors) - 1 + (step - len(input_vectors) + 1) * step_ratio) + coefficients[:, 2]
            else:
                current_vector = coefficients[:, 0] * step**2 + coefficients[:, 1] * step + coefficients[:, 2]
            output_vectors.append(current_vector)
        assert len(output_vectors) == output_count
        ...
        for dimension in range(input_vectors[0].shape[0]):
            coefficients[dimension, :] = np.polyfit(t, [latent_vector[dimension] for latent_vector in input_vectors],
                                                    3)  # highest power coefficient first
        output_vectors = list()
        for step in np.linspace(0, output_count - 1, num=output_count):
            if step >= len(input_vectors):
                current_vector = coefficients[:, 0] * (len(input_vectors) - 1 + (step - len(input_vectors) + 1) * step_ratio) ** 3 + coefficients[:, 1] * (len(input_vectors) - 1 + (step - len(input_vectors) + 1) * step_ratio) ** 2 + coefficients[:,2] * (len(input_vectors) - 1 + (step - len(input_vectors) + 1) * step_ratio) + coefficients[:, 3]
            else:
                current_vector = coefficients[:, 0] * step**3 + coefficients[:,1] * step**2 + coefficients[:,2] * step + coefficients[:, 3]
            output_vectors.append(current_vector)
        assert len(output_vectors) == output_count
...
def latentVectorExtrapolateBackward(input_vectors, output_count = 3, method = "linear", step_ratio = 1):
    """
    Function to fit a polynomial to given latent vectors and generate new latent vectors between the given one earlier in time than the given ones.
    :param input_vectors: list of 2 or more given latent vectors
    :param output_count: total number of desired output latent vectors (including the input vectors)
    :param method: extrapolation method
    :param step_ratio: distance between extrapolated vectors normalized by the distance between the input vectors
    ...
    if method == "linear" and len(input_vectors) == 2:
        ...
        for step in np.linspace(- output_count + len(input_vectors), len(input_vectors) - 1, num=output_count):
            if step >= len(input_vectors):
                current_vector = coefficients[:, 0] * (
                            len(input_vectors) - 1 + (step - len(input_vectors) + 1) * step_ratio) ** 3 + coefficients[
                                                                                                          :, 1] * (
                                             len(input_vectors) - 1 + (
                                                 step - len(input_vectors) + 1) * step_ratio) ** 2 + coefficients[:,
                                                                                                     2] * (
                                             len(input_vectors) - 1 + (
                                                 step - len(input_vectors) + 1) * step_ratio) + coefficients[:, 3]
            else:
                current_vector = coefficients[:, 0] * step ** 3 + coefficients[:, 1] * step ** 2 + coefficients[:,
                                                                                                   2] * step + coefficients[
                                                                                                               :, 3]
            output_vectors.append(current_vector)
        assert len(output_vectors) == output_count
...
if __name__ == "__main__":
    print("This module is not callable on its own")